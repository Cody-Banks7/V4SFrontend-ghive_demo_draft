<template>
    <div v-if="renderComponent">
        ...
        <svg :height="totalHeight + 80" style="margin-top: 5px; width: 100%">
...
export default {
    ...
    watch:{
        finishLoading(){
            if(this.finishLoading === true && this.counters && Object.values(this.counters).length > 0) {
        ...
        taskLength() {
            if (this.app.isTaskListProcessed)
              return
            this.app.isTaskListProcessed = true
            // console.log('taskList length===============', this.taskListAll.length)
            // console.log('taskList       ===============', this.taskListAll)
            ...
            // })
            this.taskNum = this.taskListAll.length
            this.machineNames = d3.map(this.taskListAll, d=>d.machine_id).keys()

            // TODO nest: group by, move to backend
    ...
    computed: {
        ...
        fetchData() {
            return this.app.fetchData.length === 0 ? null : this.app.fetchData
        },
        selectTaskList() {
            return [...this.selectTaskListOrigin]
        },
        taskLength() {
            return this.taskListAll.length
        },
        totalHeight(){
