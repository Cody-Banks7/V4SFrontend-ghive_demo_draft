...
// import {MSTAggrAlgo} from "@/utils/algo/mst-aggregation/MstAggregation";
// import {MCAggrAlgo} from "@/utils/algo/mst-aggregation/MCAggregation";
// import {DepthAggrAlgo} from "@/utils/algo/mst-aggregation/DepthAggregation";
import {TopologicalAggrAlgo} from "@/utils/algo/mst-aggregation/TopologicalAggregation";
...
import {loadTasksToApp} from "@/utils/application-utils/taskLoader";
...
const actions = {
    ...
    queryUpdate(context) {
        dataService.queryUpdate({}, resp => {
            ...
            //     // console.log("zero for resp",resp)
            //     return
            // }

            // console.log(resp.changed.length)
            ...
            context.commit("initialAppDiagnoseMatrix",diagnoseMatrix)
            loadTasksToApp(context.state, context.state.application, tasks)
            setTimeout(() => context.commit('layoutAppTDAG', context.state.application), 0)

            //TODO diagnose matrix
...
const mutations = {
    ...
    initialApp(state, {dataName, dagData}) {
    ...
    //     state.application.fetchData.forEach(dict => {
    //         if (dict.label === 'NORMAL') {
    //             dict.srcTask = state.application.taskMap[dict.src];
    //             dict.dstTask = state.application.taskMap[dict.dst];
    //         } else {
    //             // src are all tasks from srcVtxName
    //             dict.srcTasks = state.application.vertexMap[dict.srcVtxName].tasks
    //             dict.dstTask = state.application.taskMap[dict.dst];
    //         }
    //     })
    //     state.application.dataFlow = state.application.fetchData
    //     state.renderSign = !state.renderSign
    // },
