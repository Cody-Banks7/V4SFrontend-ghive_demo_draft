...
export default {
    ...
    props: ['app', 'taskList', 'id', 'ch', 'status', 'showDataflow',
        ...
        'abnormalDeps',
        // interactive module
        'interactiveModule', 'selectedTasks', 'selectedDeps',
        'srcAffectedTasks', 'dstAffectedTasks',
    ...
    methods: {
        click() {
            if (this.selectedTask) {
                this.taskClick(this.selectedTask)
            }
        },
        createLegends() {
        ...
        render() {
            ...
            // canvas-------------------------------
            let canvasContainer = d3.select(this.$el).select('.matrixContainer').node()
            this.ps = new ParticleSystem(canvasContainer)

            this.ps.init()
            let vertices = []
            let startTime = this.startTime == undefined ? d3.min(this.taskList, task => task.start_time): this.startTime
            let endTime = this.endTime == undefined ? d3.max(this.taskList, task => task.end_time): this.endTime
            ...
            // Add dependencies
            this.abnormalDeps.forEach(dep => {
                vertices.push(xTimeScale(dep.dstTask.start_time), yTimeScale(dep.srcTask.end_time), 0)
                colors.push(217 / 255, 95 / 255, 2 / 255)
            })
