<template>
    <g :transform="rowTransform" v-if="renderComponent">
        <rect style="rx:1; ry:1" :width="width" :height="this.data.config.unitHeight - 2 * marginY"
        ...
        <g>
            ...
            <g :transform="'translate(' + [taskDurationX, 0] + ')'">
                ...
                <!--                <PercBar v-else-->
                <!--                         :height="data.config.unitHeight" :width="taskLength" :barHeight="machineSize" :marginLR="10"-->
                <!--                         :maxVal="dataSummary[contextType][1]" :val="dataSummary[contextType][0]"-->
                <!--                         :showMax="true"-->
                <!--                ></PercBar>-->
...
export default {
    ...
    mounted(){
        ...
        keys.forEach(key=>{
            this.metricSet[key] = []
            this.data.values.forEach(machine=>{
                machine.taskList.forEach(task=>{
                    this.metricSet[key].push(this.counters[task.task_id][key])
                })
            })
            this.metricScale[key] = d3.scaleLinear().domain([0, d3.max(this.metricSet[key])]).range([0, this.taskLength])
        })
    },
    methods: {
        sum(arr) {
    ...
    computed: {
        currentStroke(){
            if(this.data.config.outerSelect == true){
                return 'purple'
            }
            return 'grey'
        },
