<template>
  <svg>
    <g class="rootContainer" >
      ...
      ></rect>
      <g class="container"  >
        <g v-if="loaded" transform="translate(100,0)">
          <DagNode v-for="node in nodes" :key="node.idx" :node="node"
...
export default {
  ...
  methods: {
    renderDagDiagram() {
      let nodes = this.app.dagData ? this.app.dagData.vertexes : null
      let edges = this.app.edges
      ...
      this.$store.commit('comparison/estimationIncrementalContainerHeight', nVertex)
